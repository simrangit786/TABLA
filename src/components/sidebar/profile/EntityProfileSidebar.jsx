...
class EntityProfileSidebar extends Component {
    state = {
        backPopup: false
    };
        ...
        if (method === methods.create) {
            return <React.Fragment>
                <Button onClick={() => this.onBackButton(true)}
                    ...
                    <span className="text-back-btn pr-3">{'Cliquez pour retourner à la liste entité TablaCasa'}</span>
                </Button>
                <h6
                    className="heading-name position-absolute  text-white pr-3">{'CRÉER UN PROFIL ENTITÉ TABLACASA '}</h6>
            ...
            return <React.Fragment>
                <Button onClick={() => history.push(reverse(routes.dashboard.profiles.entity.self))}
                        className="back-btn position-absolute bg-transparent border-0 p-0 flex-align-center">
                    <Icon type="arrow-left"/>
                    <span className="text-back-btn pr-3">{'Cliquez pour retourner à la liste entité TablaCasa'}</span>
                </Button>
            ...
            <React.Fragment>
                ...
                {params.method === methods.edit ?
                    backPopup ? <ConfirmPopup
                        ...
                        width="50%"
                        onCancel={() => history.push(reverse(routes.dashboard.profiles.entity.method,
                            {method: methods.view, id: params.id,}))}
                        okText={t('cancel')}
                        cancelText={"OUI"}
                        title={"Quitter la MODIFICATION du profil ENTITÉ TABLACASA"}
                    ...
                    backPopup ? <ConfirmPopup
                        onOk={() => this.onBackButton(false)}
                        width="50%"
                        onCancel={() => history.push(reverse(routes.dashboard.profiles.entity.self))}
                        okText={t('cancel')}
                        cancelText={"OUI"}
                        title={"Quitter la création du profil ENTITÉ TABLACASA"}
                        description={"Êtes-vous sûr de vouloir quitter la création de ce profil ?"}
                        small_description={"Vos progrès ne seront pas sauvegardés, et aucun retour arrière ne sera possible.Pour retourner à la création du profil, cliquez sur ‘Annuler’."}
                    /> : ""
                }
            </React.Fragment>
...
export default (withTranslation('common')(EntityProfileSidebar));
