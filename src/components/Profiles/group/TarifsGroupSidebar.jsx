...
class TarifsGroupSidebar extends Component {
    state = {
        backPopup: false
    };
    ...
    getComponent = () => {
        ...
        if (method === methods.create) {
            ...
            return <React.Fragment>
                <Button onClick={() => history.push(reverse(routes.dashboard.profiles.tarifs.self))}
                        className="back-btn position-absolute bg-transparent border-0 p-0 flex-align-center">
                    <Icon type="arrow-left"/>
                    <span className="text-back-btn pr-3">{t('group_click_exit')}</span>
                </Button>
            ...
            <React.Fragment>
                ...
                {params.method === methods.edit ?
                    backPopup ? <ConfirmPopup
                        ...
                        width="50%"
                        onCancel={() => history.push(reverse(routes.dashboard.profiles.tarifs.method,
                    ...
                    backPopup ? <ConfirmPopup
                        onOk={() => this.onBackButton(false)}
                        width="50%"
                        onCancel={() => history.push(reverse(routes.dashboard.profiles.tarifs.self))}
                        okText={t('cancel')}
                        cancelText={"OUI"}
                        title={"Quitter la création du profil GROUPE"}
...
export default (withTranslation('common')(TarifsGroupSidebar));