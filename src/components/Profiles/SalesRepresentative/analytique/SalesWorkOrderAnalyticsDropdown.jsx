...
class SalesWorkOrderAnalyticsDropdown extends Component {
    ...
    setTotalValues = (results = null) => {
        ...
        this.setState({total_year_amount, total_month_amount})
    ...
    OnDropDownChange = (value) => {
        const id = value.key
        if (id) {
            this.setState({loading: true, selected: value});
            salesAnalyticsWorkOrder(id).then(
                resp => {
                    const graph = resp.data.graph;
                    this.setState({results: graph, total_profiles:resp.data.total_distributor});
    ...
    render() {
        const {data, loading, options, profile, selected} = this.state;
        const {t} = this.props;
        return (
            <React.Fragment>
                <div className="row summary-info-row mx-0 p-0">
                    <div className="col-12 px-0">
                        <div className="card mb-0 shadow-none border-0 position-relative">
                            <div className="card-body">
                                <div className="row mx-0">
                                    <div className="col-md-3 col-sm-3 col-12 col-lg-3">
                                        <div className="row ana-li-que-row-data">
                                            <div className="col-12 ana-li-que-row-graph border-none-select border-0">
                                                <SalesRepresentativeFilter labelInValue className={'w-100'}
                                                                           value={selected}
...
export default withTranslation('common')(SalesWorkOrderAnalyticsDropdown);
